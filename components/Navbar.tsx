            ...
            <AnimatePresence>
                {isVisible && (
                    ...
                    >
                        <div className={`transition-all duration-500 transform-gpu ${
                            ...
                            <div className={`flex items-center justify-between gap-6 ${isFloating ? 'max-w-5xl mx-auto' : 'mx-auto max-w-7xl'}`}>
                                ...
                                <Link href="/" className="flex items-center gap-2 group shrink-0">
                                    <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-[#1E2A36] border border-[#5FA8A0]/30 text-white group-hover:border-[#5FA8A0]/60 transition-all">
                                ...
                                <nav className="hidden items-center gap-6 lg:flex">
                                    {links.map((link) => (
                                        ...
                                        >
                                            {link.name}
                                            <span className={`absolute -bottom-1 left-0 h-[2px] bg-secondary transition-all duration-300 ${pathname === link.href ? 'w-full' : 'w-0 group-hover:w-full'}`} />
                                        </Link>
                                    ))}
                                    <Link
                                        href="/contact"
                                        className="ml-2 rounded-full bg-[#F4F1EA] px-5 py-2 text-[12px] font-bold tracking-wider text-[#1A1D23] shadow-[0_4px_12px_rgba(0,0,0,0.3)] hover:shadow-[0_6px_16px_rgba(0,0,0,0.4)] transition-all active:scale-95 whitespace-nowrap"
                                    >
                                        Book Now
                                    </Link>
                                </nav>
                        ...
                        {!isFloating && (
                            <div
                                className="absolute bottom-0 left-0 h-[2px] bg-secondary transition-all duration-150 ease-out"
            ...
            <div className={`fixed inset-y-0 right-0 z-50 w-full max-w-sm border-l border-[rgba(255,255,255,0.05)] bg-[#121212]/95 backdrop-blur-3xl transition-transform duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] lg:hidden transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                <div className="flex flex-col h-full p-8">
                    ...
                    <div className="mt-auto">
                        <Link href="/contact" className="group relative overflow-hidden flex w-full items-center justify-between rounded-xl bg-[#F4F1EA] p-5 text-sm font-bold tracking-wider text-[#1A1D23] shadow-[0_4px_12px_rgba(0,0,0,0.3)] hover:shadow-[0_6px_16px_rgba(0,0,0,0.4)] transition-all">
